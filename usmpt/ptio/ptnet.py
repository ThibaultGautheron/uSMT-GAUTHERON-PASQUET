...
class PetriNet:
    ...
    def smtlib_set_initial_marking(self, k: Optional[int] = None) -> str:
        ...
        for pl in self.places:
            var = pl if k is None else "{}@{}".format(pl, k)
            smt_input += "(= {} {}) ".format(var,self.initial_marking[pl])
    ...
    # TODO: Sect. 2.3.1. #
    ######################
    def smtlib_transition_enable(self, k: int, tr:str) -> str:
        
        smt_input = "(and "
        for pl in self.places:
            var = pl if k is None else "{}@{}".format(pl, k)
            smt_input += "(>= {} {}) ".format(var,self.pre[tr].get(pl,0))
        smt_input += ")"
        return smt_input
    
    def smtlib_transition_diff(self, k: int, k_prime: int,tr:str) -> str:
       
        smt_input = "(and "
        for pl in self.places:
            var = pl if k is None else "{}@{}".format(pl, k)
            varp = pl if k_prime is None else "{}@{}".format(pl, k_prime)
            smt_input += " (= (+ {} {}) {}) ".format(var, -self.pre[tr].get(pl,0) + self.post[tr].get(pl,0), varp)
        smt_input += ")"
        return smt_input
    ...
    def smtlib_transition_relation(self, k: int, k_prime: int) -> str:
        ...
        for t in self.transitions:
            smt_input += "\n\t(and {} {})".format(self.smtlib_transition_diff(k,k_prime,t),self.smtlib_transition_enable(k,t))
        smt_input +="))"
